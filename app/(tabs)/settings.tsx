...
export default function SettingsScreen() {
  ...
  return (
    <ScreenContainer className="px-5">
      ...
      <MediaRestorePicker visible={showMediaPicker} files={encryptedMedia.data ?? []} selectedIds={selectedMediaIds} busy={restoreEncryptedMedia.isPending} onClose={() => setShowMediaPicker(false)} onToggle={toggleMedia} onRestore={() => { setShowMediaPicker(false); void runMediaRestore(selectedMediaIds); }} />
    </ScreenContainer>
  );
}
...
function MediaRestorePicker({ visible, files, selectedIds, busy, onClose, onToggle, onRestore }: { visible: boolean; files: Array<{ id: number; fileName: string; contentType: string; originalSize: number; createdAt: Date | string }>; selectedIds: number[]; busy: boolean; onClose: () => void; onToggle: (id: number) => void; onRestore: () => void }) {

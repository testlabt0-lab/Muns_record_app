...
export default function SettingsScreen() {
  ...
  const { years, terms, subjects, lectures, reviewCards, reviewLists, reviewSessions, reviewChallenges, tasks, subjectGoals, weeklySubjectGoals, syncSettings, updateSyncSettings, replaceStoreFromBackup, replaceStoreFromLocalImport, replaceSubjectGoalSettings, restoreReplacementSnapshot, deleteReplacementSnapshot, updateLecture, addAttachment, backupActivities, replacementSnapshots, addBackupActivity } = useStudy();
  ...
  const [pendingImport, setPendingImport] = useState<StudyDataImportPayload | null>(null);
  const [pendingImportFileName, setPendingImportFileName] = useState("");
  const [importSelection, setImportSelection] = useState<StudyDataImportSection[]>(["structure", "lectures", "review", "tasks"]);
  const [importingStudyData, setImportingStudyData] = useState(false);
  const [goalSettingsBusy, setGoalSettingsBusy] = useState(false);
  ...
  const exportStudyData = async () => {
    if (exportingStudyData) return;
    setExportingStudyData(true);
    try {
      const json = createStudyDataExportJson({ years, terms, subjects, lectures, reviewCards, reviewLists, reviewSessions, reviewChallenges, tasks, syncSettings, backupActivities });
      const fileName = `muhadir-study-data-${new Date().toISOString().slice(0, 10)}.json`;
      if (Platform.OS === "web") {
        const href = URL.createObjectURL(new Blob([json], { type: "application/json" }));
        const link = document.createElement("a"); link.href = href; link.download = fileName; document.body.appendChild(link); link.click(); link.remove(); URL.revokeObjectURL(href);
        Alert.alert("تم تنزيل البيانات", "حُفِظ ملف JSON محلياً في جهازك.");
        return;
      }
      if (!FileSystem.cacheDirectory) throw new Error("تعذر الوصول إلى مساحة الملفات المؤقتة.");
      const uri = `${FileSystem.cacheDirectory}${fileName}`;
      await FileSystem.writeAsStringAsync(uri, json, { encoding: FileSystem.EncodingType.UTF8 });
      if (!(await Sharing.isAvailableAsync())) { Alert.alert("تم تجهيز الملف", "تم إنشاء ملف JSON محلياً، لكن المشاركة غير متاحة على هذا الجهاز."); return; }
      await Sharing.shareAsync(uri, { UTI: "public.json", mimeType: "application/json", dialogTitle: "مشاركة بيانات مُحاضِر" });
    } catch { Alert.alert("تعذر تصدير البيانات", "تحقق من مساحة الجهاز ثم أعد المحاولة."); }
    finally { setExportingStudyData(false); }
  };
  const exportGoalSettings = async () => { if (goalSettingsBusy) return; setGoalSettingsBusy(true); try { const json = createSubjectGoalsBackupJson(subjectGoals ?? [], weeklySubjectGoals ?? []); const fileName = `muhadir-subject-goals-${new Date().toISOString().slice(0, 10)}.json`; if (Platform.OS === "web") { const href = URL.createObjectURL(new Blob([json], { type: "application/json" })); const link = document.createElement("a"); link.href = href; link.download = fileName; document.body.appendChild(link); link.click(); link.remove(); URL.revokeObjectURL(href); Alert.alert("تم تنزيل إعدادات الأهداف", "حُفِظ ملف JSON محلياً في جهازك."); return; } if (!FileSystem.cacheDirectory) throw new Error("تعذر الوصول إلى مساحة الملفات المؤقتة."); const uri = `${FileSystem.cacheDirectory}${fileName}`; await FileSystem.writeAsStringAsync(uri, json, { encoding: FileSystem.EncodingType.UTF8 }); if (!(await Sharing.isAvailableAsync())) { Alert.alert("تم تجهيز الملف", "تم إنشاء ملف الأهداف محلياً، لكن المشاركة غير متاحة على هذا الجهاز."); return; } await Sharing.shareAsync(uri, { UTI: "public.json", mimeType: "application/json", dialogTitle: "مشاركة إعدادات أهداف مُحاضِر" }); } catch { Alert.alert("تعذر تصدير الأهداف", "حاول مرة أخرى بعد التحقق من مساحة الجهاز."); } finally { setGoalSettingsBusy(false); } };
  const restoreGoalSettings = async () => { if (goalSettingsBusy) return; setGoalSettingsBusy(true); try { const result = await DocumentPicker.getDocumentAsync({ type: ["application/json", "text/json"], copyToCacheDirectory: true }); if (result.canceled) return; const asset = result.assets[0]; const webFile = asset.file as { text?: () => Promise<string> } | undefined; const source = Platform.OS === "web" && webFile?.text ? await webFile.text() : await FileSystem.readAsStringAsync(asset.uri, { encoding: FileSystem.EncodingType.UTF8 }); const restored = restoreSubjectGoalsForKnownSubjects(parseSubjectGoalsBackupJson(source), subjects.map((subject) => subject.id)); Alert.alert("استعادة إعدادات الأهداف", `سيُستبدل إعداد الهدف الفصلي لـ ${restored.subjectGoals.length} مادة، وإعداد الأسبوع لـ ${restored.weeklySubjectGoals.length} هدف. لا تتأثر المحاضرات أو التسجيلات أو البطاقات.`, [{ text: "إلغاء", style: "cancel" }, { text: "استعادة", onPress: () => { replaceSubjectGoalSettings(restored.subjectGoals, restored.weeklySubjectGoals); Alert.alert("تمت الاستعادة", "أعيدت إعدادات الأهداف للمواد المعروفة على هذا الجهاز."); } }]); } catch (error) { Alert.alert("تعذرت استعادة الأهداف", error instanceof Error ? error.message : "اختر ملف JSON صحيحاً صادرًا من مُحاضِر."); } finally { setGoalSettingsBusy(false); } };
  const chooseAppearanceMode = (mode: "light" | "dark") => { setColorScheme(mode); updateSyncSettings({ appearanceMode: mode }); };
  const chooseStudyDataFile = async () => {
    if (importingStudyData) return;
    try {
      ...
      const source = Platform.OS === "web" && webFile?.text ? await webFile.text() : await FileSystem.readAsStringAsync(asset.uri, { encoding: FileSystem.EncodingType.UTF8 });
      setPendingImport(parseStudyDataImportJson(source)); setPendingImportFileName(asset.name); setImportSelection(["structure", "lectures", "review", "tasks"]);
    } catch (error) { Alert.alert("تعذر قراءة ملف البيانات", error instanceof Error ? error.message : "اختر ملف JSON صحيحاً صادرًا من مُحاضِر."); }
  };
  const toggleImportSection = (section: StudyDataImportSection) => setImportSelection((current) => current.includes(section) ? current.filter((item) => item !== section) : [...current, section]);
  const applyStudyDataImport = () => {
  ...
  const replaceStudyDataImport = () => {
    ...
    Alert.alert("استبدال بيانات الدراسة", "سيُستبدل تنظيم الدراسة والمحاضرات والنصوص والمراجعة والمهام بهذا الملف. تُحفظ تلقائياً لقطة قبل الاستبدال ويمكن استعادتها من هذه الصفحة. ستُؤرشف أي محاضرة محلية تحمل ملفات صوت أو مرفقات ولا تظهر في الملف لحماية وسائط جهازك.", [{ text: "إلغاء", style: "cancel" }, { text: "استبدال البيانات", style: "destructive", onPress: () => void (async () => { setImportingStudyData(true); try { const next = replaceStudyDataFromImport({ years, terms, subjects, lectures, reviewCards, reviewLists, reviewSessions, reviewChallenges, tasks, syncSettings, backupActivities, replacementSnapshots }, pendingImport); replaceStoreFromLocalImport(next, `حالة قبل استبدال «${pendingImportFileName}»`); addBackupActivity({ action: "restore", status: "completed", message: `استُبدلت بيانات الدراسة من «${pendingImportFileName}» مع أرشفة الوسائط المحلية غير المطابقة.` }); setPendingImport(null); Alert.alert("تم استبدال البيانات", "اعتمد التطبيق البيانات من الملف، وحُفظت لقطة محلية للاستعادة عند الحاجة."); } catch { Alert.alert("تعذر استبدال البيانات", "لم تُغيّر بيانات الجهاز. حاول بملف صادر من مُحاضِر."); } finally { setImportingStudyData(false); } })() }]);
  };
  ...
  return (
    <ScreenContainer className="px-5">
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <View style={styles.exportCard}><View style={styles.limitHeading}><MaterialIcons name="file-download" size={21} color={appTheme.primary} /><View style={styles.rowText}><Text style={styles.rowTitle}>تصدير بيانات الدراسة</Text><Text style={styles.rowDetail}>ينشئ ملف JSON محلياً يضم التنظيم والنصوص والملخصات والمراجعة. لا يضم ملفات الصوت أو المرفقات نفسها.</Text></View><StatusPill label="محلي" tone="success" /></View><Pressable disabled={exportingStudyData} onPress={() => void exportStudyData()} style={[styles.exportDataButton, exportingStudyData && styles.disabled]}><MaterialIcons name="ios-share" size={18} color="#FFFFFF" /><Text style={styles.exportDataText}>{exportingStudyData ? "يجري تجهيز الملف" : "تصدير ومشاركة ملف JSON"}</Text></Pressable></View>
        <View style={styles.exportCard}><View style={styles.limitHeading}><MaterialIcons name="flag" size={21} color={appTheme.warning} /><View style={styles.rowText}><Text style={styles.rowTitle}>نسخة إعدادات الأهداف</Text><Text style={styles.rowDetail}>صدّر أو استعد أهداف الترم والأسبوع فقط. لا تتغير المحاضرات أو التسجيلات أو بطاقات المراجعة.</Text></View><StatusPill label="محلي" tone="success" /></View><View style={styles.backupActions}><Pressable disabled={goalSettingsBusy} onPress={() => void restoreGoalSettings()} style={[styles.secondaryButton, goalSettingsBusy && styles.disabled]}><Text style={styles.secondaryButtonText}>استعادة الأهداف</Text></Pressable><Pressable disabled={goalSettingsBusy} onPress={() => void exportGoalSettings()} style={[styles.exportDataButton, goalSettingsBusy && styles.disabled]}><MaterialIcons name="ios-share" size={18} color="#FFFFFF" /><Text style={styles.exportDataText}>{goalSettingsBusy ? "يجري التجهيز" : "تصدير أهداف JSON"}</Text></Pressable></View></View>
        <View style={styles.exportCard}><View style={styles.limitHeading}><MaterialIcons name="file-upload" size={21} color={appTheme.success} /><View style={styles.rowText}><Text style={styles.rowTitle}>استيراد بيانات مُحاضِر</Text><Text style={styles.rowDetail}>اختر ملف JSON صادرًا من مُحاضِر ثم حدّد الفئات التي تريد دمجها. لا تستبدل بيانات جهازك الحالية.</Text></View><StatusPill label="انتقائي" tone="primary" /></View><Pressable disabled={importingStudyData} onPress={() => void chooseStudyDataFile()} style={[styles.restoreMediaButton, importingStudyData && styles.disabled]}><MaterialIcons name="file-upload" size={18} color={appTheme.success} /><Text style={styles.restoreMediaText}>{importingStudyData ? "يجري الدمج" : "اختيار ملف JSON للاستيراد"}</Text></Pressable></View>
        {(replacementSnapshots ?? []).length ? <View style={styles.limitCard}><View style={styles.limitHeading}><MaterialIcons name="restore-page" size={21} color={appTheme.warning} /><View style={styles.rowText}><Text style={styles.rowTitle}>سجل استبدال البيانات</Text><Text style={styles.rowDetail}>تُحفظ آخر 3 حالات قبل الاستبدال محلياً. الاستعادة تنشئ لقطة أمان جديدة للحالة الحالية.</Text></View><StatusPill label={`${(replacementSnapshots ?? []).length} لقطات`} tone="primary" /></View>{(replacementSnapshots ?? []).map((snapshot) => <View key={snapshot.id} style={styles.activityRow}><Pressable onPress={() => Alert.alert("استعادة لقطة", `هل تريد استعادة «${snapshot.label}»؟ سيُحتفظ بلقطة أمان للحالة الحالية.`, [{ text: "إلغاء", style: "cancel" }, { text: "استعادة", onPress: () => restoreReplacementSnapshot(snapshot.id) }])} style={styles.restoreSnapshotButton}><MaterialIcons name="restore" size={17} color="#FFFFFF" /><Text style={styles.restoreSnapshotText}>استعادة</Text></Pressable><View style={styles.rowText}><Text style={styles.activityText}>{snapshot.label}</Text><Text style={styles.activityTime}>{new Date(snapshot.createdAt).toLocaleString("ar")}</Text></View><Pressable onPress={() => deleteReplacementSnapshot(snapshot.id)}><MaterialIcons name="close" size={17} color={appTheme.danger} /></Pressable></View>)}</View> : null}
        <View style={styles.limitCard}><View style={styles.limitHeading}><MaterialIcons name="dark-mode" size={21} color={appTheme.violet} /><View style={styles.rowText}><Text style={styles.rowTitle}>مظهر التطبيق</Text><Text style={styles.rowDetail}>اختر المظهر الفاتح أو الليلي. يُحفظ اختيارك على هذا الجهاز.</Text></View><StatusPill label={colorScheme === "dark" ? "ليلي" : "فاتح"} tone="primary" /></View><View style={styles.sortRow}><Pressable onPress={() => chooseAppearanceMode("dark")} style={[styles.sortChip, colorScheme === "dark" && styles.sortChipActive]}><MaterialIcons name="dark-mode" size={16} color={appTheme.violet} /><Text style={[styles.sortChipText, colorScheme === "dark" && styles.sortChipTextActive]}>ليلي</Text></Pressable><Pressable onPress={() => chooseAppearanceMode("light")} style={[styles.sortChip, colorScheme === "light" && styles.sortChipActive]}><MaterialIcons name="light-mode" size={16} color={appTheme.violet} /><Text style={[styles.sortChipText, colorScheme === "light" && styles.sortChipTextActive]}>فاتح</Text></Pressable></View></View>
...
export function StudyDataImportPicker({ visible, fileName, payload, selected, busy, onClose, onToggle, onApply }: { visible: boolean; fileName: string; payload: StudyDataImportPayload | null; selected: StudyDataImportSection[]; busy: boolean; onClose: () => void; onToggle: (section: StudyDataImportSection) => void; onApply: () => void }) {
  const details: Record<StudyDataImportSection, { label: string; note: string; count: number }> = { structure: { label: "هيكل الدراسة", note: "السنوات والترم والمواد", count: (payload?.years.length ?? 0) + (payload?.terms.length ?? 0) + (payload?.subjects.length ?? 0) }, lectures: { label: "المحاضرات والنصوص", note: "دون الصوت والمرفقات", count: payload?.lectures.length ?? 0 }, review: { label: "المراجعة", note: "البطاقات والجلسات والقوائم", count: (payload?.reviewCards.length ?? 0) + (payload?.reviewSessions.length ?? 0) + (payload?.reviewLists.length ?? 0) }, tasks: { label: "الواجبات والخطة", note: "المهام دون تنبيهات النظام", count: payload?.tasks.length ?? 0 } };
  return <Modal visible={visible} animationType="slide" presentationStyle="pageSheet" onRequestClose={onClose}><View style={styles.modal}><View style={styles.modalHeader}><Pressable onPress={onClose} style={styles.closeButton}><MaterialIcons name="close" size={22} color={appTheme.ink} /></Pressable><Text style={styles.modalTitle}>دمج بيانات محلية</Text><View style={styles.closeButton} /></View><ScrollView contentContainerStyle={importStyles.content}><View style={importStyles.notice}><MaterialIcons name="privacy-tip" size={21} color={appTheme.success} /><View style={styles.rowText}><Text style={styles.rowTitle} numberOfLines={1}>{fileName}</Text><Text style={styles.rowDetail}>لا تُستبدل بياناتك الحالية ولا يمكن استعادة ملفات الوسائط من ملف JSON.</Text></View></View><Text style={importStyles.instruction}>اختر الفئات التي تريد إضافتها إلى هذا الجهاز:</Text>{importSections.map((section) => { const chosen = selected.includes(section); const detail = details[section]; return <Pressable key={section} disabled={busy} onPress={() => onToggle(section)} style={[importStyles.choice, chosen && importStyles.choiceSelected, busy && styles.disabled]}><View style={[importStyles.check, chosen && importStyles.checkSelected]}>{chosen ? <MaterialIcons name="check" size={15} color="#FFFFFF" /> : null}</View><View style={styles.rowText}><Text style={styles.rowTitle}>{detail.label}</Text><Text style={styles.rowDetail}>{detail.note} · {detail.count} عنصر</Text></View><MaterialIcons name="chevron-left" size={20} color={appTheme.muted} /></Pressable>; })}</ScrollView><View style={styles.modalFooter}><PrimaryButton label={busy ? "يجري دمج البيانات" : `دمج ${selected.length} فئات`} icon="merge-type" disabled={!selected.length || busy} onPress={onApply} /></View></View></Modal>;
}
...
const styles = StyleSheet.create({
  ...
  exportCard: { backgroundColor: "#F0FDFA", borderColor: "#99F6E4", borderRadius: 18, borderWidth: 1, gap: 12, padding: 14 }, exportDataButton: { alignItems: "center", backgroundColor: appTheme.primary, borderRadius: 13, flexDirection: "row-reverse", gap: 7, justifyContent: "center", minHeight: 45 }, exportDataText: { color: "#FFFFFF", fontSize: 12, fontWeight: "800" }, restoreSnapshotButton: { alignItems: "center", backgroundColor: appTheme.primary, borderRadius: 9, flexDirection: "row-reverse", gap: 4, justifyContent: "center", minHeight: 33, paddingHorizontal: 9 }, restoreSnapshotText: { color: "#FFFFFF", fontSize: 10, fontWeight: "800" },
});

const importStyles = StyleSheet.create({
  content: { gap: 10, padding: 18, paddingBottom: 92 }, notice: { alignItems: "flex-start", backgroundColor: appTheme.successSoft, borderRadius: 15, flexDirection: "row-reverse", gap: 10, padding: 13 }, instruction: { color: appTheme.muted, fontSize: 12, fontWeight: "700", marginTop: 4, textAlign: "right" }, choice: { alignItems: "center", backgroundColor: appTheme.surface, borderColor: appTheme.border, borderRadius: 15, borderWidth: 1, flexDirection: "row-reverse", gap: 10, padding: 13 }, choiceSelected: { backgroundColor: "#F5F3FF", borderColor: appTheme.primary }, check: { alignItems: "center", borderColor: "#CBD5E1", borderRadius: 9, borderWidth: 1, height: 22, justifyContent: "center", width: 22 }, checkSelected: { backgroundColor: appTheme.primary, borderColor: appTheme.primary },
});

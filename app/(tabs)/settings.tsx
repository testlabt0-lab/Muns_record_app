...
export default function SettingsScreen() {
  ...
  return (
    <ScreenContainer className="px-5">
      ...
      <Modal visible={showMediaPicker} animationType="slide" presentationStyle="pageSheet" onRequestClose={() => setShowMediaPicker(false)}><View style={styles.modal}><View style={styles.modalHeader}><Pressable onPress={() => setShowMediaPicker(false)} style={styles.closeButton}><MaterialIcons name="close" size={22} color={appTheme.ink} /></Pressable><Text style={styles.modalTitle}>اختر الملفات لاستعادتها</Text><View style={styles.closeButton} /></View><ScrollView contentContainerStyle={styles.modalList}>{(encryptedMedia.data ?? []).map((file) => { const chosen = selectedMediaIds.includes(file.id); return <Pressable key={file.id} onPress={() => toggleMedia(file.id)} style={[styles.mediaChoice, chosen && styles.mediaChoiceSelected]}><View style={[styles.check, chosen && styles.checkSelected]}>{chosen ? <MaterialIcons name="check" size={15} color="#FFFFFF" /> : null}</View><View style={styles.rowText}><Text style={styles.mediaName} numberOfLines={1}>{file.fileName}</Text><Text style={styles.mediaMeta}>{file.contentType.startsWith("audio/") ? "تسجيل صوتي" : "مرفق"} · {formatBytes(file.originalSize)}</Text></View><MaterialIcons name={file.contentType.startsWith("audio/") ? "graphic-eq" : "attach-file"} size={20} color={appTheme.primary} /></Pressable>; })}</ScrollView><View style={styles.modalFooter}><PrimaryButton label={`استعادة ${selectedMediaIds.length} ملف`} icon="lock-open" disabled={!selectedMediaIds.length || restoreEncryptedMedia.isPending} onPress={() => { setShowMediaPicker(false); void runMediaRestore(selectedMediaIds); }} /></View></View></Modal>
    </ScreenContainer>
  );
}

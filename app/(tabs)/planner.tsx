...
export default function PlannerScreen() {
  ...
  const addToCalendar = async (task: StudyTask) => {
    ...
    try {
      ...
      await Calendar.createEventInCalendarAsync({ title: task.title, startDate, endDate: new Date(startDate.getTime() + 60 * 60 * 1000), notes: `أضيف من تطبيق مُحاضِر · ${kindLabel[task.kind]}` }, {});
    } catch { Alert.alert("تعذر فتح التقويم", "تحقق من أذونات التقويم ثم حاول مرة أخرى."); }
  };

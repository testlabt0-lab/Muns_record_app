...
export default function PlannerScreen() {
  ...
  const closeForm = () => { setCreating(false); setTitle(""); setDueDate(new Date(Date.now() + 24 * 60 * 60 * 1000)); setKind("assignment"); setSubjectId(undefined); };
  const saveTask = async () => {
    ...
    try { const notificationId = await scheduleStudyReminder(title.trim(), dueDate.toISOString(), id); if (notificationId) updateTask(id, { notificationId }); } catch { /* يظل الموعد محفوظًا محليًا إن تعذر منح إذن التنبيهات. */ }
    closeForm();
  };
  const toggleTask = async (task: StudyTask) => { updateTask(task.id, { completed: !task.completed }); if (!task.completed) await cancelStudyReminder(task.notificationId); };

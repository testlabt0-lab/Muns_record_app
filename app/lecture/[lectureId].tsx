...
import type { LectureAttachment, TranscriptLanguage } from "@/lib/study-types";
...
export default function LectureDetailScreen() {
  ...
  const createSummary = async () => {
    ...
    try {
      ...
      const summary = await summarize.mutateAsync({ transcript, language: lecture.transcriptionLanguage === "en" ? "en" : "ar" });
  ...
  const exportPdf = async () => {
    if (!getLectureTranscript(lecture) && !lecture.summary) { Alert.alert("لا يوجد محتوى للتصدير", "حوّل المحاضرة إلى نص أو أنشئ ملخصاً أولاً."); return; }

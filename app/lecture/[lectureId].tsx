...
export default function LectureDetailScreen() {
  ...
  const totalPlaybackDuration = useMemo(() => Math.max(lecture?.durationSeconds ?? 0, audioParts.reduce((total, part) => total + part.durationSeconds, 0)), [audioParts, lecture?.durationSeconds]);
  ...
  useEffect(() => {
    if (!lecture || !totalPlaybackDuration) return;
    if (playerStatus.didJustFinish && activePartIndex === audioParts.length - 1) {
      if (lecture.lastPlaybackPositionSeconds !== undefined) updateLecture(lecture.id, { lastPlaybackPositionSeconds: undefined, lastPlaybackUpdatedAt: undefined });
      return;
    }
    if (!playerStatus.playing) return;
    const beforeCurrentPart = audioParts.slice(0, activePartIndex).reduce((total, part) => total + part.durationSeconds, 0);
    const position = normalizeResumablePlaybackPosition(beforeCurrentPart + playerStatus.currentTime, totalPlaybackDuration);
    if (position === undefined) return;
    if (lecture.lastPlaybackPositionSeconds === undefined || Math.abs(lecture.lastPlaybackPositionSeconds - position) >= 10) updateLecture(lecture.id, { lastPlaybackPositionSeconds: position, lastPlaybackUpdatedAt: new Date().toISOString() });
  }, [activePartIndex, audioParts, lecture, playerStatus.currentTime, playerStatus.didJustFinish, playerStatus.playing, totalPlaybackDuration, updateLecture]);
  useEffect(() => {
    if (!lecture) return;
  ...
  const addNote = () => {
    try {
      ...
      if (editingNoteId) {
        updateLecture(lecture.id, { notes: (lecture.notes ?? []).map((note) => note.id === editingNoteId ? updateLectureNote(note, noteInput, now) : note) });
        setNoteInput("");
        setEditingNoteId(undefined);
        return;
      }
      const partOffset = audioParts.slice(0, activePartIndex).reduce((total, part) => total + part.durationSeconds, 0);
      const totalDuration = Math.max(lecture.durationSeconds, audioParts.reduce((total, part) => total + part.durationSeconds, 0));
      const note = createLectureNote({ id: `note-${Date.now()}`, text: noteInput, timestampSeconds: activePart ? partOffset + playerStatus.currentTime : undefined, durationSeconds: totalDuration, now });
      updateLecture(lecture.id, { notes: [note, ...(lecture.notes ?? [])] });
      setNoteInput("");
    } catch (error) { Alert.alert("تعذر حفظ الملاحظة", error instanceof Error ? error.message : "حاول مرة أخرى."); }
  };

  const beginEditNote = (noteId: string) => {
    const note = (lecture.notes ?? []).find((item) => item.id === noteId);
    if (!note) return;
    setEditingNoteId(note.id);
    setNoteInput(note.text);
  };

  const cancelEditNote = () => { setEditingNoteId(undefined); setNoteInput(""); };
  const removeNote = (noteId: string) => {
    updateLecture(lecture.id, { notes: (lecture.notes ?? []).filter((note) => note.id !== noteId) });
  ...
  const exportPdf = async () => {
    if (!lecture.transcript && !lecture.summary && !(lecture.notes ?? []).length) { Alert.alert("لا يوجد محتوى للتصدير", "أضف ملاحظات أو حوّل المحاضرة إلى نص أو أنشئ ملخصاً أولاً."); return; }
    try { await exportLecturePdf(lecture, subject); } catch { Alert.alert("تعذر التصدير", "حاول إنشاء ملف PDF مرة أخرى."); }
  };

  const shareActiveAudio = async () => {
    const part = getActiveAudioPartForShare(audioParts, activePartIndex);
    if (!part) { Alert.alert("لا يوجد تسجيل", "أضف أو استورد تسجيلاً قبل مشاركته."); return; }
    if (Platform.OS === "web") { Alert.alert("يتوفر على الهاتف", "مشاركة ملف التسجيل تعمل عبر ورقة المشاركة الأصلية في تطبيق الهاتف."); return; }
    if (sharingAudio) return;
    try {
      const file = new File(part.uri);
      ...
      await Sharing.shareAsync(part.uri, { UTI: "public.audio", mimeType: getAudioShareMimeType(part.uri), dialogTitle: `مشاركة ${lecture.title} · الجزء ${activePartIndex + 1}` });
    } catch (error) { Alert.alert("تعذرت مشاركة التسجيل", error instanceof Error ? error.message : "حاول مرة أخرى."); }
    finally { setSharingAudio(false); }
  };
  ...
  const resumePosition = getResumePlaybackPosition(lecture.lastPlaybackPositionSeconds, totalPlaybackDuration);
  ...
  return <ScreenContainer className="px-5"><AppHeader eyebrow={subject?.title ?? "محاضرة"} title={lecture.title} action={<IconButton icon="arrow-forward" label="رجوع" onPress={() => router.back()} tone="neutral" />} /><ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
    <View style={styles.audioCard}>
      ...
      {resumePosition !== undefined ? <Pressable accessibilityRole="button" accessibilityLabel={`استئناف المحاضرة من ${formatDuration(resumePosition)}`} onPress={() => playTranscriptSegment(resumePosition)} style={styles.resumePlayback}><MaterialIcons name="play-circle-outline" size={18} color="#C7D2FE" /><Text style={styles.resumePlaybackText}>استئناف من {formatDuration(resumePosition)}</Text></Pressable> : null}
      {activePart ? <Pressable accessibilityRole="button" accessibilityLabel="مشاركة الجزء الصوتي الحالي" disabled={sharingAudio} onPress={() => void shareActiveAudio()} style={[styles.shareAudio, sharingAudio && styles.disabled]}><MaterialIcons name="share" size={18} color="#C7D2FE" /><Text style={styles.shareAudioText}>{sharingAudio ? "يجري فتح المشاركة" : `مشاركة ${audioParts.length > 1 ? `الجزء ${activePartIndex + 1}` : "التسجيل"}`}</Text></Pressable> : null}
    ...
    <View style={styles.attachmentsCard}><View style={styles.attachmentsHeader}><Text style={styles.sectionTitle}>ملاحظاتي</Text><StatusPill label={`${(lecture.notes ?? []).length} ملاحظة`} tone="neutral" /></View><Text style={styles.attachmentPrivacy}>{editingNoteId ? "عدّل النص ثم احفظه؛ سيبقى الموضع الزمني الأصلي للملاحظة." : "اكتب ملاحظة أثناء الاستماع؛ يربطها التطبيق بموضع التشغيل الحالي عند توفر تسجيل."}</Text><TextInput value={noteInput} onChangeText={setNoteInput} multiline textAlignVertical="top" placeholder="مثل: ارجع إلى هذا المثال قبل الاختبار" placeholderTextColor="#94A3B8" textAlign="right" accessibilityLabel={editingNoteId ? "تعديل ملاحظة المحاضرة" : "ملاحظة جديدة للمحاضرة"} style={styles.noteInput} /><View style={styles.attachmentTop}><Text style={styles.attachmentActionText}>{editingNoteId ? "سيبقى الموضع الزمني دون تغيير" : activePart ? `سيرتبط الحفظ بالموضع ${formatDuration(Math.floor(audioParts.slice(0, activePartIndex).reduce((sum, part) => sum + part.durationSeconds, 0) + playerStatus.currentTime))}` : "ستُحفظ الملاحظة دون موضع صوتي"}</Text><View style={styles.noteActions}>{editingNoteId ? <Pressable accessibilityRole="button" accessibilityLabel="إلغاء تعديل الملاحظة" onPress={cancelEditNote} style={styles.noteCancel}><Text style={styles.noteCancelText}>إلغاء</Text></Pressable> : null}<Pressable accessibilityRole="button" accessibilityLabel={editingNoteId ? "حفظ تعديل الملاحظة" : "حفظ ملاحظة للمحاضرة"} onPress={addNote} style={styles.extractButton}><MaterialIcons name={editingNoteId ? "save" : "note-add"} size={17} color={appTheme.primary} /><Text style={styles.extractButtonText}>{editingNoteId ? "حفظ التعديل" : "حفظ ملاحظة"}</Text></Pressable></View></View>{(lecture.notes ?? []).map((note) => <View key={note.id} style={styles.attachmentRow}><View style={styles.attachmentTop}>{note.timestampSeconds === undefined ? <View style={styles.attachmentOpen}><MaterialIcons name="notes" size={19} color={appTheme.primary} /><Text style={styles.segmentTime}>ملاحظة عامة</Text></View> : <Pressable accessibilityRole="button" accessibilityLabel={`تشغيل من ${formatDuration(note.timestampSeconds)}`} onPress={() => playTranscriptSegment(note.timestampSeconds ?? 0)} style={styles.attachmentOpen}><MaterialIcons name="play-circle-outline" size={19} color={appTheme.primary} /><Text style={styles.segmentTime}>{formatDuration(note.timestampSeconds)}</Text></Pressable>}<View style={styles.noteActions}><Pressable accessibilityRole="button" accessibilityLabel="تعديل ملاحظة" onPress={() => beginEditNote(note.id)} style={styles.attachmentEdit}><MaterialIcons name="edit" size={16} color={appTheme.primary} /></Pressable><Pressable accessibilityRole="button" accessibilityLabel="حذف ملاحظة" onPress={() => removeNote(note.id)} style={styles.attachmentDelete}><MaterialIcons name="close" size={17} color={appTheme.danger} /></Pressable></View></View><Text style={styles.noteText}>{note.text}</Text></View>)}</View>

    <Pressable disabled={!lecture.transcript && !lecture.summary && !(lecture.notes ?? []).length} onPress={exportPdf} style={({ pressed }) => [styles.exportButton, (!lecture.transcript && !lecture.summary && !(lecture.notes ?? []).length) && styles.disabled, pressed && styles.pressed]}><MaterialIcons name="picture-as-pdf" size={20} color={appTheme.primary} /><Text style={styles.exportText}>تصدير محتوى المحاضرة PDF</Text></Pressable>
...
const styles = StyleSheet.create({
  ...
  partTabs: { flexDirection: "row-reverse", gap: 6, marginBottom: 12, overflow: "hidden" }, partTab: { backgroundColor: "#334155", borderRadius: 9, paddingHorizontal: 10, paddingVertical: 7 }, partTabActive: { backgroundColor: "#E0E7FF" }, partTabText: { color: "#CBD5E1", fontSize: 11, fontWeight: "800" }, partTabTextActive: { color: appTheme.primary }, speedRow: { alignItems: "center", flexDirection: "row-reverse", gap: 6, justifyContent: "center", marginBottom: 12 }, speedLabel: { color: "#CBD5E1", fontSize: 11, fontWeight: "800", marginLeft: 4 }, speedChip: { backgroundColor: "#334155", borderRadius: 9, paddingHorizontal: 8, paddingVertical: 6 }, speedChipActive: { backgroundColor: "#E0E7FF" }, speedChipText: { color: "#CBD5E1", fontSize: 11, fontWeight: "800" }, speedChipTextActive: { color: appTheme.primary }, partLabel: { color: "#CBD5E1", fontSize: 11, marginBottom: 8, textAlign: "center" }, resumePlayback: { alignItems: "center", borderColor: "#64748B", borderRadius: 11, borderWidth: 1, flexDirection: "row-reverse", gap: 6, justifyContent: "center", marginBottom: 9, minHeight: 38 }, resumePlaybackText: { color: "#E0E7FF", fontSize: 12, fontWeight: "800" }, shareAudio: { alignItems: "center", borderColor: "#64748B", borderRadius: 11, borderWidth: 1, flexDirection: "row-reverse", gap: 6, justifyContent: "center", marginBottom: 9, minHeight: 38 }, shareAudioText: { color: "#E0E7FF", fontSize: 12, fontWeight: "800" },
  attachmentsCard: { backgroundColor: appTheme.surface, borderColor: appTheme.border, borderRadius: 19, borderWidth: 1, gap: 11, padding: 14 }, attachmentsHeader: { alignItems: "center", flexDirection: "row-reverse", justifyContent: "space-between" }, attachmentPrivacy: { color: appTheme.muted, fontSize: 11, lineHeight: 17, textAlign: "right" }, attachmentActions: { flexDirection: "row-reverse", gap: 7 }, attachmentAction: { alignItems: "center", backgroundColor: appTheme.primarySoft, borderRadius: 11, flex: 1, gap: 4, justifyContent: "center", minHeight: 57, paddingHorizontal: 3 }, attachmentActionText: { color: appTheme.primary, fontSize: 10, fontWeight: "800", textAlign: "center" }, attachmentRow: { backgroundColor: "#F8FAFC", borderRadius: 12, gap: 9, padding: 10 }, noteInput: { backgroundColor: "#F8FAFC", borderColor: appTheme.border, borderRadius: 12, borderWidth: 1, color: appTheme.ink, fontSize: 13, lineHeight: 20, minHeight: 78, padding: 10, textAlign: "right" }, noteText: { color: appTheme.ink, fontSize: 13, lineHeight: 21, textAlign: "right" }, attachmentTop: { alignItems: "center", flexDirection: "row-reverse", gap: 7 }, attachmentOpen: { alignItems: "center", flex: 1, flexDirection: "row-reverse", gap: 7 }, attachmentTitle: { color: appTheme.ink, flex: 1, fontSize: 12, fontWeight: "700", textAlign: "right" }, attachmentDelete: { alignItems: "center", backgroundColor: appTheme.dangerSoft, borderRadius: 9, height: 29, justifyContent: "center", width: 29 }, attachmentEdit: { alignItems: "center", backgroundColor: appTheme.primarySoft, borderRadius: 9, height: 29, justifyContent: "center", width: 29 }, noteActions: { alignItems: "center", flexDirection: "row-reverse", gap: 6 }, noteCancel: { alignItems: "center", backgroundColor: "#E2E8F0", borderRadius: 10, justifyContent: "center", minHeight: 38, paddingHorizontal: 10 }, noteCancelText: { color: appTheme.muted, fontSize: 11, fontWeight: "800" }, extractButton: { alignItems: "center", backgroundColor: appTheme.primarySoft, borderRadius: 10, flexDirection: "row-reverse", gap: 6, justifyContent: "center", minHeight: 38 }, extractButtonText: { color: appTheme.primary, fontSize: 11, fontWeight: "800" }, extractionError: { color: appTheme.danger, fontSize: 11, lineHeight: 16, textAlign: "right" }, extractionResult: { backgroundColor: "#FFFFFF", borderColor: "#C7D2FE", borderRadius: 10, borderWidth: 1, gap: 7, padding: 10 }, extractionLabel: { color: appTheme.primary, fontSize: 11, fontWeight: "800", textAlign: "right" }, extractionText: { color: appTheme.ink, fontSize: 12, lineHeight: 19, textAlign: "right" },

...
import { createLectureNote } from "@/lib/lecture-notes";
...
export default function LectureDetailScreen() {
  ...
  const addNote = () => {
    try {
      ...
      const note = createLectureNote({ id: `note-${Date.now()}`, text: noteInput, timestampSeconds: activePart ? partOffset + playerStatus.currentTime : undefined, durationSeconds: totalDuration, now: new Date().toISOString() });
  ...
  const removeNote = (noteId: string) => updateLecture(lecture.id, { notes: (lecture.notes ?? []).filter((note) => note.id !== noteId) });
  ...
  return <ScreenContainer className="px-5"><AppHeader eyebrow={subject?.title ?? "محاضرة"} title={lecture.title} action={<IconButton icon="arrow-forward" label="رجوع" onPress={() => router.back()} tone="neutral" />} /><ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
    ...
    <View style={styles.attachmentsCard}><View style={styles.attachmentsHeader}><Text style={styles.sectionTitle}>ملاحظاتي</Text><StatusPill label={`${(lecture.notes ?? []).length} ملاحظة`} tone="neutral" /></View><Text style={styles.attachmentPrivacy}>اكتب ملاحظة أثناء الاستماع؛ يربطها التطبيق بموضع التشغيل الحالي عند توفر تسجيل.</Text><TextInput value={noteInput} onChangeText={setNoteInput} multiline textAlignVertical="top" placeholder="مثل: ارجع إلى هذا المثال قبل الاختبار" placeholderTextColor="#94A3B8" textAlign="right" accessibilityLabel="ملاحظة جديدة للمحاضرة" style={styles.noteInput} /><View style={styles.attachmentTop}><Text style={styles.attachmentActionText}>{activePart ? `سيرتبط الحفظ بالموضع ${formatDuration(Math.floor(audioParts.slice(0, activePartIndex).reduce((sum, part) => sum + part.durationSeconds, 0) + playerStatus.currentTime))}` : "ستُحفظ الملاحظة دون موضع صوتي"}</Text><Pressable accessibilityRole="button" accessibilityLabel="حفظ ملاحظة للمحاضرة" onPress={addNote} style={styles.extractButton}><MaterialIcons name="note-add" size={17} color={appTheme.primary} /><Text style={styles.extractButtonText}>حفظ ملاحظة</Text></Pressable></View>{(lecture.notes ?? []).map((note) => <View key={note.id} style={styles.attachmentRow}><View style={styles.attachmentTop}>{note.timestampSeconds === undefined ? <View style={styles.attachmentOpen}><MaterialIcons name="notes" size={19} color={appTheme.primary} /><Text style={styles.segmentTime}>ملاحظة عامة</Text></View> : <Pressable accessibilityRole="button" accessibilityLabel={`تشغيل من ${formatDuration(note.timestampSeconds)}`} onPress={() => playTranscriptSegment(note.timestampSeconds ?? 0)} style={styles.attachmentOpen}><MaterialIcons name="play-circle-outline" size={19} color={appTheme.primary} /><Text style={styles.segmentTime}>{formatDuration(note.timestampSeconds)}</Text></Pressable>}<Pressable accessibilityRole="button" accessibilityLabel="حذف ملاحظة" onPress={() => removeNote(note.id)} style={styles.attachmentDelete}><MaterialIcons name="close" size={17} color={appTheme.danger} /></Pressable></View><Text style={styles.noteText}>{note.text}</Text></View>)}</View>

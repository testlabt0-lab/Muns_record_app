...
export default function LectureDetailScreen() {
  ...
  const addTag = () => {
    ...
    updateLecture(lecture.id, { tags: [...(lecture.tags ?? []), tag], tagColors: { ...(lecture.tagColors ?? {}), [tag]: tagColor } });
    setTagInput("");
  };
  ...
  const extractAttachmentText = async (attachment: LectureAttachment) => {
    ...
    try {
      ...
      updateAttachment(lecture.id, attachment.id, { extractionStatus: "completed", extractedText: result.text, extractionKeyPoints: result.keyPoints, extractedAt: new Date().toISOString(), extractionError: undefined });
    } catch (error) {

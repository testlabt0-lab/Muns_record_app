...
describe("أهداف المادة الأسبوعية", () => {
  it("يحفظ قيماً سليمة ويحسب مراجعة وتركيز المادة منذ بداية الأسبوع", () => {
    expect(normalizeWeeklyGoalTargets({ reviewTarget: -1, focusMinutesTarget: 30 })).toEqual({ reviewTarget: 0, focusMinutesTarget: 30, lateReminderThresholdPercent: 50 });
  ...
  it("يحدد تأخر الهدف بعد منتصف الأسبوع فقط وبلا تكرار بعد التنبيه", () => {
    ...
    expect(isWeeklyGoalLate({ reviewedCardCount: 6, focusMinutes: 36 }, { ...goal, lateReminderThresholdPercent: 70 }, new Date("2026-01-08"))).toBe(true);
  });
});

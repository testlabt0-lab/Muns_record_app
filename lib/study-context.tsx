...
import { createFollowUpFilterPreset, type FollowUpFilterPresetInput } from "@/lib/weekly-reflection-follow-up-filter-presets";
...
type StudyContextValue = StudyStore & {
  ...
  replaceFollowUpActivities: (activities: FollowUpActivity[]) => void;
  saveFollowUpFilterPreset: (title: string, filters: FollowUpFilterPresetInput) => string | undefined;
  deleteFollowUpFilterPreset: (presetId: string) => void;
...
export function StudyProvider({ children }: { children: React.ReactNode }) {
  ...
  const value = useMemo<StudyContextValue>(() => {
    ...
    return {
      ...
      replaceFollowUpActivities: (activities) => setStore((current) => ({ ...current, followUpActivities: activities ?? [] })),
      saveFollowUpFilterPreset: (title, filters) => { const id = makeId("follow-up-filter"); const preset = createFollowUpFilterPreset(id, title, filters, new Date().toISOString()); if (!preset) return undefined; setStore((current) => ({ ...current, followUpFilterPresets: [preset, ...(current.followUpFilterPresets ?? [])].slice(0, 8) })); return id; },
      deleteFollowUpFilterPreset: (presetId) => setStore((current) => ({ ...current, followUpFilterPresets: (current.followUpFilterPresets ?? []).filter((preset) => preset.id !== presetId) })),
      saveWeeklyReflection: (weekStart, note, attributes) => setStore((current) => { const timestamp = new Date().toISOString(); const previous = (current.weeklyReflections ?? []).find((reflection) => reflection.weekStart === weekStart); const normalized = normalizeWeeklyReflection(weekStart, note, { ...attributes, followUpCreatedAt: previous?.followUpCreatedAt }, timestamp); const saved = normalized.followUpGoal ? { ...normalized, followUpOverdueReminderNotificationId: previous?.followUpOverdueReminderNotificationId, followUpOverdueReminderScheduleKey: previous?.followUpOverdueReminderScheduleKey } : normalized; const remaining = (current.weeklyReflections ?? []).filter((reflection) => reflection.weekStart !== weekStart); const nextRepeat = createNextWeeklyFollowUpRepeat(saved); const alreadyExists = Boolean(nextRepeat && remaining.some((reflection) => reflection.weekStart === nextRepeat.weekStart)); const draft = getFollowUpActivityDraft(previous, saved, timestamp); const activities = draft ? [{ ...draft, id: makeId("follow-up-activity") }, ...(current.followUpActivities ?? [])].slice(0, 200) : (current.followUpActivities ?? []); return { ...current, weeklyReflections: [saved, ...(nextRepeat && !alreadyExists ? [nextRepeat] : []), ...remaining], followUpActivities: activities }; }),

...
export async function scheduleWeeklyReflectionFollowUpOverdueReminder(goal: string, dueAt: string, time: FollowUpOverdueReminderTime) { const trigger = getFollowUpOverdueReminderSchedule(dueAt, time); if (Platform.OS === "web" || !trigger) return undefined; if (Platform.OS === "android") await Notifications.setNotificationChannelAsync("study-follow-up-overdue", { name: "خطوات متابعة متأخرة", importance: Notifications.AndroidImportance.DEFAULT }); const current = await Notifications.getPermissionsAsync(); const permission = current.status === "granted" ? current : await Notifications.requestPermissionsAsync(); if (permission.status !== "granted") return undefined; return Notifications.scheduleNotificationAsync({ content: { title: "لديك خطوة متابعة متأخرة", body: goal, data: { url: "/follow-up-steps" } }, trigger: { type: Notifications.SchedulableTriggerInputTypes.DATE, date: trigger, channelId: "study-follow-up-overdue" } }); }
export async function cancelWeeklyReflectionFollowUpOverdueReminder(notificationId?: string) { if (notificationId && Platform.OS !== "web") await Notifications.cancelScheduledNotificationAsync(notificationId); }

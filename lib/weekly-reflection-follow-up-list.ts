...
export function filterOpenFollowUpItemsBySubject(items: OpenFollowUpItem[], subjectId: string) { if (subjectId === "all") return items; if (subjectId === "unlinked") return items.filter((item) => !item.followUpSubjectId); return items.filter((item) => item.followUpSubjectId === subjectId); }
